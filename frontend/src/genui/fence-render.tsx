...
import { completeFenceJson, describeJsonFailure, isCompleteJson, repairFenceJson } from './fence-repair.ts'
...
/** 兜底代码块样式（原 dsh CodeBlock 替代）。 */
const MONO_PRE_STYLE: CSSProperties = {
  margin: '6px 0',
  padding: '10px 12px',
  borderRadius: 8,
  background: 'var(--dsw-color-surface-2, rgba(127,127,127,0.08))',
  border: '1px solid var(--dsw-color-border, rgba(127,127,127,0.2))',
...
function FenceFallback({ raw, fenceKey }: { raw: string; fenceKey: Key }) {
  ...
  return (
    <div ref={ref}>
      ...
      <pre key={fenceKey} style={{ ...MONO_PRE_STYLE }} data-genui-fence-raw>{raw}</pre>
    </div>
  )
}
