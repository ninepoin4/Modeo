...
function Bubble({ m, streaming, delay }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* 2026-08-18：用户消息也提供复制（此前只有助手消息有） */}
        {m.content && !streaming && (
          <button
            data-testid={isUser ? 'copy-user-msg' : 'copy-msg'}
            onClick={copy}
            className="absolute right-2 top-2 rounded-md p-1 opacity-0 transition-opacity hover:bg-paper2 hover:text-ink group-hover/msg:opacity-100"
            title="复制"
          >
